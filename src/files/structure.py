...
class SPHParser():
    ...
    def __init__(self, name:str='none', QUIET:bool=True):
        ...
        # self.n_residues = 0 # This is for handling residue number overflow in the output files. Because there is only one atom per residue, n_atoms should equal n_residues.
    ...
    # def _get_residue_num(self, value:str):
...
class StructureFile():
    ...
    def from_file(cls, path, format:str=None):
        ...
        assert format in ['cif', 'pdb', 'sph'], 'StructureFile.from_file: Format must be either pdb, cif, or sph.'
        
        parser = StructureFile.parsers[format](QUIET=True)
    ...
    def to_df(self, atoms:list=ATOMS, residues=AMINO_ACID_CODES):
        ...
        for chain in self.structure.get_chains():
            for residue in chain.get_residues():
                ...
                # assert len(b_factors) == 1, f'StructureFile.to_df: Expected one b factor per residue, but got {b_factors}.' 
    ...
    def subset(self, output_path:str, start:int=None, stop:int=None, chain_id:str='A'):
        assert start is not None, f'StructureFile.subset: The start and stop indices need to be numbers, got {type(start)}.'
        assert stop is not None, f'StructureFile.subset: The start and stop indices need to be numbers, got {type(start)}.'
        io = PDB.PDBIO()
        io.set_structure(self.structure)
...
def cif_to_pdb(path:str, output_path:str=None, output_dir:str=None, overwrite:bool=False):
    ...
    if (not os.path.exists(output_path)) or overwrite:
        file = StructureFile.from_file(path, format='cif')
...
def sph_to_pdb(path:str, output_dir:str=None, overwrite:bool=False):
    ...
    if (not os.path.exists(output_path)) or overwrite:
        file = StructureFile.from_file(path, format='sph')
        file.write(output_path)
    return output_path
